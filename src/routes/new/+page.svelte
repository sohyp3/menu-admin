	...
	<div class="p-6 mx-auto max-w-3xl">
		...
		<div class="flex flex-col gap-6 p-6 bg-teal-100 rounded-lg shadow-md">
			...
			{#each data.categories as category}
				...
				>
					<a href="/new/parent_category/{category.id}" class="col-span-1 text-left text-gray-800">{category.id}</a>
					<a href="/new/parent_category/{category.id}" class="col-span-2 text-left text-gray-800">{category.name['tr']}</a>
					<a href="/new/parent_category/{category.id}" class="col-span-2 text-left text-gray-800">{category.name['en']}</a>
				</div>
			{/each}
		</div>
	</div>
